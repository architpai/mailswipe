...
const SwipeableCard = React.forwardRef(function SwipeableCard({ email, onSwipe, onOpenDetail }, ref) {
  ...
  return (
    ...
    >
      ...
      {/* KEEP — Green frosted glass overlay */}
      <motion.div
        className="absolute inset-0 rounded-2xl flex items-center justify-center pointer-events-none bg-green-500/30 backdrop-blur-sm"
        style={{ opacity: keepOpacity }}
      >
        <span className="text-white font-bold text-4xl tracking-widest border-2 border-white/40 px-6 py-2 rounded-xl transform rotate-12 bg-white/10 backdrop-blur-sm">
          KEEP
        </span>
      </motion.div>

      {/* TRASH — Red frosted glass overlay */}
      <motion.div
        className="absolute inset-0 rounded-2xl flex items-center justify-center pointer-events-none bg-red-500/30 backdrop-blur-sm"
      ...
      {/* ARCHIVE — Blue frosted glass overlay */}
      <motion.div
        className="absolute inset-0 rounded-2xl flex items-start justify-center pt-10 pointer-events-none bg-blue-500/30 backdrop-blur-sm"
        style={{ opacity: archiveOpacity }}
      >
        <span className="text-white font-bold text-4xl tracking-widest border-2 border-white/40 px-6 py-2 rounded-xl bg-white/10 backdrop-blur-sm">
          ARCHIVE
        </span>
      </motion.div>
    </motion.div>
